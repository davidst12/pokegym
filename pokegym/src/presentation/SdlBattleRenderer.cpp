...
auto SdlBattleRenderer::onBattleEvent(const pokegym::battle::BattleEvent& event) -> void {
    std::cout << "SdlBattleRenderer received event: " << static_cast<int>(event.index())
              << std::endl;
...
auto SdlBattleRenderer::processEvent(const pokegym::battle::BattleStartsEvent& event) -> void {
    dialog_text_ = "You are challenged by Rival " + event.opponent_name + "!";
}

auto SdlBattleRenderer::processEvent(const pokegym::battle::PokemonSentOutEvent& event) -> void {
...
auto SdlBattleRenderer::processEvent(const pokegym::battle::ActionRequestEvent& event) -> void {
...
auto SdlBattleRenderer::processEvent(const pokegym::battle::MoveUsedEvent& event) -> void {
    if (event.side == pokegym::battle::Side::Player) {
...
auto SdlBattleRenderer::processEvent(const pokegym::battle::DamageEvent& event) -> void {
    if (event.side == pokegym::battle::Side::Player) {
...
auto SdlBattleRenderer::processEvent(const pokegym::battle::PokemonFaintedEvent& event) -> void {
    if (event.side == pokegym::battle::Side::Player) {
...
auto SdlBattleRenderer::processEvent(const pokegym::battle::BattleWinEvent& event) -> void {
    if (event.side == pokegym::battle::Side::Player) {
    ...
    switch (action_animation_.state) {
        ...
        case ActionRequestAnimation::State::ShowMoves:
            if (selection >= 1 && selection <= 4) {
                Action action{.trainer_type = battle::Action::TrainerType::Player,
                              .type = battle::Action::Type::Attack,
                              .index = selection - 1};
